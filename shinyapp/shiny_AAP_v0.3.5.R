...
ui_plot1 <- fluidPage(
  ...
  fluidRow(
    column(6,selectInput("dataset", "Plot type", choices = c('box','violin','bar','halves','line','line_noer','heatmap'))),
...
ui_axis <- fluidPage(
  ...
  fluidRow(
    column(4,selectInput("er", "Error bar", choices = c('se','sd'))),
    column(4,selectInput("jitter", "Jitter", choices = c('no','yes'))),
    column(4,selectInput("theme", "Theme", choices = c('classic','bw','bw_blank')))
    ),
  fluidRow(
    column(4,selectInput("sigmethod", "Sigmethod", choices = c('none','anova')))
  ),
  fluidRow(
...
server <- function(input, output, session) {
  ...
  # Plot -------------------------------------------------------
  pldata <- reactive({
    DVisual <- function(data, palette = '',
                        type = 'box', group_type = 'none', theme = 'bw', jitter = F, sigmethod = F, sigdata, er='se',
