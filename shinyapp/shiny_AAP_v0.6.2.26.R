    ...
    DVana <- function(data, method = 'duncan'){
      ...
      if (method %in% c('duncan', 'lsd', 'tukey')) {
        if ('facet' %in% colnames(data)) {
          ...
          resulte <-  data.frame(group="",mean="",sd="",se="",label="",max="",min="",facet = "")[-1,]
          for (i in c(1:length(unique(data$facet)))){
            ...
            min <- result$means[,ifelse(method == 'lsd',7,5)]
            marker<-sort$mar.groups
            facet <- rep(facetgroup1,length(rowname))
            result2<-data.frame(rowname,mean,sd,se,marker,max,min,facet)
            colnames(result2) <- c('group','mean','sd','se','label','max','min','facet')
          ...
          min <- result$means[,ifelse(method == 'lsd',7,5)]
          marker<-sort$mar.groups
          result2<-data.frame(rowname,mean,sd,se,marker,max,min)
          colnames(result2) <- c('group','mean','sd','se','label','max','min')
          return(result2)
        }
