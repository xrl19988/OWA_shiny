...
# ---- download anova result
ui_download <- fluidRow(
  column(width = 12, downloadButton(outputId = "download", label = "Download ANOVA results!", class = "btn-block")))
...
ui_axis <- fluidPage(
  fluidRow(
    ...
    sliderInput("siglab", "Sig label position", min = 0, max = 1, value = 1),
...
server <- function(input, output, session) {
  ...
  # Plot -------------------------------------------------------
  pldata <- reactive({
    DVisual <- function(data, palette = df,
                        type = 'box', group_type = 'none', theme = 'bw', jitter = F, sigmethod = F, sigdata,
      ...
      if (group_type %in% c('none','facet')){
        if (type == 'box') {
          p1 <- ggplot(data = data,aes(x=group,y = value))+
            geom_boxplot(aes(color=group),fill="white",width=0.5,size = 0.7,alpha= 1,position = position_dodge(1),notch = F,
                         outlier.colour = "black", outlier.size=1.5, outlier.alpha = 1)+
          ...
          p1 <- ggplot(data = data, mapping = aes(x = group,y = value, color = group))+
            ...
            stat_summary(fun.y='mean',geom='point',shape=5,size=2,color="black") # 标记均值
        ...
        if (group_type == 'none') {
          ...
          p2 <- p1+
            facet_wrap(~facet,scales = "free_y",2,2)
        }
        # jitter
        if (jitter == T) {
          p3 <- p2+
        ...
        if (type == 'box') {
          p1 <- ggplot(data = data,aes(x=facet,y = value,fill=group))+
            geom_boxplot(aes(fill=group),width=0.5,size = 0.7,alpha= 1,notch = F,position = position_dodge(width=0.8),
                         outlier.colour = "black", outlier.size=1.5, outlier.alpha = 1)+
            stat_summary(fun.y='mean',geom='point',shape=5,size=1.5,color='black',position = position_dodge(0.8))
          ...
          p1 <- ggplot(data = data, mapping = aes(x = facet,y = value, color = group))+
            ...
            geom_boxplot(aes(color=group),alpha= 0.5, outlier.size=1,outlier.alpha = 0,outlier.color = "black",
                         width=0.1,size = 0.8,position = position_dodge(1))+
            stat_halfeye(mapping = aes(fill=group),width = 0.25, justification = -0.65, .width = 0,point_colour = NA)+ # 中位数
            stat_summary(fun.y='mean',geom='point',shape=5,size=2,color="black",position = position_dodge(0.8)) # 标记均值
          # scale_y_continuous(expand = c(0.05,0.05)
          #                    # ,limits = c(22,35)
          # )+
          # facet_grid(sample~.)
    ...
    pldata <- DVisual(data = melt(), type = input$dataset, group_type = input$gtype, theme = 'classic', jitter = F,
                       sigmethod = 'anova', sigdata = tidied(),x_axis=input$x,y_axis=input$y)
  })
